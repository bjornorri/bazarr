...
from app.event_handler import event_stream, hide_progress, show_progress
...
def store_subtitles(original_path, reversed_path, use_cache=True):
    ...
    if os.path.exists(reversed_path):
        ...
        matching_episodes = database.execute(
            select(TableEpisodes.sonarrEpisodeId).where(
                TableEpisodes.path == original_path
            )
        ).all()
...
def series_full_scan_subtitles(use_cache=None):
    ...
    episodes = database.execute(select(TableEpisodes.path)).all()

    count_episodes = len(episodes)
    for i, episode in enumerate(episodes):
        show_progress(
            id="episodes_disk_scan",
